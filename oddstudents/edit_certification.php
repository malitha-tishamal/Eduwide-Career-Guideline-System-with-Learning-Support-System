...
$stmt = $conn->prepare("SELECT * FROM former_students_certifications WHERE id = ? AND former_student_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['certification_image']['name'])) {
        ...
        if (in_array($image_ext, $allowed) && $_FILES['certification_image']['size'] < 5000000) {
            ...
            $stmt = $conn->prepare("UPDATE former_students_certifications SET certification_name = ?, issued_by = ?, date = ?, link = ?, certification_description = ?, image_path = ? WHERE id = ? AND former_student_id = ?");
        ...
        $stmt = $conn->prepare("UPDATE former_students_certifications SET certification_name = ?, issued_by = ?, date = ?, link = ?, certification_description = ? WHERE id = ? AND former_student_id = ?");
