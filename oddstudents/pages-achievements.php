...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['event_image'])) {
    ...
    if (in_array($image_ext, $allowed_extensions) && $image_size < 5000000 && $image_error == 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO former_students_achievements (former_student_id, event_name, organized_by, event_date, event_title, event_description, image_path) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("SELECT * FROM former_students_achievements WHERE former_student_id = ?");
        $stmt->bind_param("i", $current_user_id);
        $stmt->execute();
