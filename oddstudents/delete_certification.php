...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT image_path FROM former_students_certifications WHERE id = ? AND former_student_id = ?");
    ...
    if ($cert) {
        ...
        $stmt = $conn->prepare("DELETE FROM former_students_certifications WHERE id = ? AND former_student_id = ?");
        $stmt->bind_param("ii", $certification_id, $current_user_id);
        $stmt->execute();
        $stmt->close();
    }
