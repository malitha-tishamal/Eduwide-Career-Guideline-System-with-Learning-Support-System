...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        empty($year) || empty($student_id) || empty($subject) || empty($semester) ||
        $practical_marks === '' || $paper_marks === ''
    ) {
    ...
    if ($stmt->execute()) {
        ...
        // If MySQL unique constraint failed
        if ($conn->errno === 1062) {
            $_SESSION['status'] = 'error';
            $_SESSION['message'] = 'Duplicate entry. Marks already exist.';
        } else {
            $_SESSION['status'] = 'error';
